...
const AboutComponent = (props) => {
  ...
  const renderPersonalPreviewCard = (type) => {
    switch (type) {
      ...
      case "industry":
        return (<TouchableOpacity style={styles.badge2}>
          <Ionicons name="logo-bitbucket" size={20} color="black" />
          <Text style={styles.badgeText}>{personalData[type]}</Text>
        </TouchableOpacity>)
      case "organization":
        return (<TouchableOpacity style={styles.badge2}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View>
            ...
            <Text
              style={styles.para}
            >{`${personalData?.currentLocation} ,${personalData?.jobTitle}`}</Text>
          </View>
          ...
          <View>
            ...
            <View style={styles.button}>
              <TouchableOpacity style={styles.interest}>
                <Text style={styles.badgeText}>{personalData?.lookingFor}</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <Text style={styles.subnames}>Avards & Achievements</Text>
          <View style={{ display: 'flex', justifyContent: 'space-between', marginHorizontal: 5, marginVertical: 5, marginLeft: 20, }}>
            {
              personalData.awardsAndAchievements.map((item, ind) => {
                return (
                  <Text style={{ marginVertical: 5 }}>{ind + 1 + " " + item}</Text>
                )
              })
            }
          </View>
